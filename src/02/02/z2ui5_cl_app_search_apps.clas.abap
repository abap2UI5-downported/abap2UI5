...
*        view_display( client ).
  ...
  METHOD view_display.
    ...
    DATA temp8 TYPE REF TO lcl_github.
...
*                                                  select = client->_event_client( action = 'NAV_TO' t_arg  = value #( ( `NavCon` ) ( `${$parameters}` ) ) )
                                                 select = client->_event_client( val = client->cs_event-nav_container_to t_arg  = temp4 )
                                                 mode = `Inline`
                                 )->items(
...
*            )->get_parent( )->get_parent(
    ...
    CREATE OBJECT temp8 TYPE lcl_github.
    lt_repos = temp8->get_repositories( ).
