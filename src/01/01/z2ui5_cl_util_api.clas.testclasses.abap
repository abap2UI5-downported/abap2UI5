...
CLASS ltcl_test_app IMPLEMENTATION.
  ...
  METHOD class_constructor.
    DATA temp46 TYPE ltcl_test_app=>ty_row.
    DATA temp47 LIKE st_tab.
...
CLASS ltcl_unit_test_abap_api IMPLEMENTATION.
  ...
  METHOD test_classdescr.
    ...
    DATA lo_app TYPE REF TO ltcl_test_app.
    DATA temp48 TYPE REF TO cl_abap_classdescr.
    DATA lt_attri LIKE temp48->attributes.
    ...
    temp48 ?= cl_abap_objectdescr=>describe_by_object_ref( lo_app ).
    
    lt_attri = temp48->attributes.
    ...
    temp50 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `MT_TAB` INTO temp49.
    sy-tabix = temp50.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp52 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `MV_VAL` INTO temp51.
    sy-tabix = temp52.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp54 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `SS_TAB` INTO temp53.
    sy-tabix = temp54.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp56 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `ST_TAB` INTO temp55.
    sy-tabix = temp56.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp58 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `SV_STATUS` INTO temp57.
    sy-tabix = temp58.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp60 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `SV_VAR` INTO temp59.
    sy-tabix = temp60.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_eledescr_rel_name.
    ...
    DATA temp61 TYPE REF TO cl_abap_elemdescr.
    DATA lo_ele LIKE temp61.
    temp61 ?= cl_abap_elemdescr=>describe_by_data( abap_true ).
...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_url_param_get_tab.
    ...
    DATA lt_param TYPE z2ui5_if_types=>ty_t_name_value.
    DATA temp63 LIKE LINE OF lt_param.
    ...
    temp64 = sy-tabix.
    READ TABLE lt_param WITH KEY n = `sap-client` INTO temp63.
    sy-tabix = temp64.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp66 = sy-tabix.
    READ TABLE lt_param WITH KEY n = `app_start` INTO temp65.
    sy-tabix = temp66.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_trans_xml_2_any__w_data.
    ...
    DATA temp80 TYPE ty_row.
    DATA ls_row LIKE temp80.
    DATA temp81 TYPE ty_row.
    DATA ls_row2 LIKE temp81.
    DATA lv_xml TYPE string.
    CLEAR temp80.
...
DATA BEGIN OF ms_struc2.INCLUDE TYPE ty_struc.INCLUDE TYPE ty_struc_incl.DATA END OF ms_struc2.
    ...
    DATA temp82 TYPE REF TO cl_abap_datadescr.
    ...
    lt_attri = z2ui5_cl_util=>rtti_get_t_attri_by_include( type = temp82 attri = `` ).
