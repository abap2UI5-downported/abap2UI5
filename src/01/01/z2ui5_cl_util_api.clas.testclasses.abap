...
CLASS ltcl_test_app IMPLEMENTATION.
  ...
  METHOD class_constructor.
    DATA temp51 TYPE ltcl_test_app=>ty_row.
    DATA temp52 LIKE st_tab.
...
CLASS ltcl_unit_test_abap_api IMPLEMENTATION.
  ...
  METHOD test_classdescr.
    ...
    DATA lo_app TYPE REF TO ltcl_test_app.
    DATA temp53 TYPE REF TO cl_abap_classdescr.
    DATA lt_attri LIKE temp53->attributes.
    ...
    temp53 ?= cl_abap_objectdescr=>describe_by_object_ref( lo_app ).
    
    lt_attri = temp53->attributes.
    ...
    temp55 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `MT_TAB` INTO temp54.
    sy-tabix = temp55.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp57 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `MV_VAL` INTO temp56.
    sy-tabix = temp57.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp59 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `SS_TAB` INTO temp58.
    sy-tabix = temp59.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp61 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `ST_TAB` INTO temp60.
    sy-tabix = temp61.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp63 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `SV_STATUS` INTO temp62.
    sy-tabix = temp63.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp65 = sy-tabix.
    READ TABLE lt_attri WITH KEY name = `SV_VAR` INTO temp64.
    sy-tabix = temp65.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_eledescr_rel_name.
    ...
    temp66 ?= cl_abap_elemdescr=>describe_by_data( abap_true ).
...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_url_param_get_tab.
    ...
    temp69 = sy-tabix.
    READ TABLE lt_param WITH KEY n = `sap-client` INTO temp68.
    sy-tabix = temp69.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp71 = sy-tabix.
    READ TABLE lt_param WITH KEY n = `app_start` INTO temp70.
    sy-tabix = temp71.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
...
DATA BEGIN OF ms_struc2.INCLUDE TYPE ty_struc.INCLUDE TYPE ty_struc_incl.DATA END OF ms_struc2.
    ...
    DATA temp87 TYPE REF TO cl_abap_datadescr.
    ...
    lt_attri = z2ui5_cl_util=>rtti_get_t_attri_by_include( temp87 ).
